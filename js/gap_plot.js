...
class GapPlot {
     ...
     */
    constructor(data, updateCountry, updateYear, activeYear, activeCountry) {
        ...
        this.minSize = {
            'population': Infinity,
            'gdp': Infinity,
            'child-mortality': Infinity,
            'life-expectancy': Infinity,
            'fertility-rate': Infinity
        };
        this.maxSize = {
            'population': -Infinity,
            'gdp': -Infinity,
       ...
       for (let key of d3.keys(this.data)){
          ...
          // console.log(key);
            ...
            for (let eachCountry of karray){
                //console.log(key);
                //console.log(eachCountry);
                for (let i = 1800; i <=2020; i++) {
                    let cminvalue = eachCountry[i] ? eachCountry[i] : Infinity;
                    Minvalue = cminvalue < Minvalue ? cminvalue : Minvalue;
                    let cmaxvalue = eachCountry[i] ? eachCountry[i] : -Infinity;
                    Maxvalue = cmaxvalue > Maxvalue ? cmaxvalue : Maxvalue;
                }
                //console.log(cminvalue,cmaxvalue,key);
            }
    ...
    drawPlot() {
        ...
        this.showactiveYear = this.svgGroup.append("text");
        this.showactiveYear.attr("x", 150 )
            .attr("y", 100)
    ...
    updatePlot(activeYear, xIndicator, yIndicator, circleSizeIndicator) {
        ...
        */
         ...
         */
        let minv = this.minSize[circleSizeIndicator];
        let maxv = this.maxSize[circleSizeIndicator];
        ...
        //YOUR CODE HERE  
            //*** Structuring your PlotData objects ***
        let plot_Data = [];
        let countryIDs = d3.map(this.data[xIndicator], function (d) {
            return d.geo;
        });
        //console.log(countryIDs);
        //console.log(keys);
        for (let cID of d3.keys(countryIDs)){
            let country = countryIDs[cID].country ;
            let id = countryIDs[cID].geo;
            if (id == undefined){
                continue;
            }
            //console.log(id);
            let xVal = countryIDs[cID][activeYear] !== undefined ? countryIDs[cID][activeYear] : undefined ;
            //console.log(xVal);
            let yVal = this.data[yIndicator].find(d => d.geo == id);
                    yVal = yVal ? yVal[activeYear]: undefined;
                    //console.log(yIndicator);
                   // console.log(yVal);
            let region = this.data["population"].find(d=> d.geo == id);
                region = region ? region.region : undefined;
                //console.log(region);
                //console.log(this.data["population"]);
            let cSize = this.data[circleSizeIndicator].find(d => d.geo == id);
                cSize = cSize ? cSize[activeYear] : undefined;
                //console.log(cSize);
            plot_Data.push(new PlotData(country, xVal, yVal, id, region, cSize));
        }
        //console.log(plot_Data);
        //console.log(this.maxSize[xIndicator]);
        ...
        scatterplot.attr('cx', d => (this.margin.left + (d.xVal ? xScale(d.xVal) : 0)))
            .attr('cy', d => (this.margin.top + (d.yVal ? yScale(d.yVal) : this.height)))
            .attr('r',  circleSizer)
            .attr("id", d => (d.id.toUpperCase()))
            .append('title')
            .html(this.tooltipRender);
        if (this.activeCountry != null){
            let selectedCountry = scatterplot.filter(d => (d.id)=== this.activeCountry.toUpperCase());
            selectedCountry.classed("selected-country", true);
            let selectedRegion = selectedCountry.datum().region;
            scatterplot.filter(d => (d.region != selectedRegion)).classed("hidden", true);
            scatterplot.filter(d => (d.region === selectedRegion)).classed("selected-region", true);
        }
        else {
            scatterplot.attr('class', d => 'circle ' + d.region);
        }

        let xAxisLabel = this.data[xIndicator][0].indicator_name;
        let yAxisLabel = this.data[yIndicator][0].indicator_name;
        ...
        */
        ...
        yearSlider.on('input', function(e) {
           ...
           // console.log(this.activeCountry);
            ...
            that.showactiveYear.html(activeYear);
            that.updatePlot(activeYear, xValue, yValue, cValue);
            that.updateCountry();
    ...
    updateHighlightClick(activeCountry) {
        ...
        */
        ...
        this.clearHighlight();
        //console.log(activeCountry);
        let AllCircles = d3.select(".plot-svg").selectAll("circle");
        let selectedCountry = AllCircles.filter(d => (d.id.toUpperCase()=== activeCountry.toUpperCase()));

        //console.log(selectedCountry.datum());
        let selectedRegion = selectedCountry.datum().region;
        //console.log(selectedRegion);
        selectedCountry.classed("selected-country", true);
        AllCircles.filter(d => (d.region != selectedRegion)).classed("hidden", true);
        AllCircles.filter(d => (d.region === selectedRegion)).classed("selected-region", true);
